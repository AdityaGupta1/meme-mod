...
public class EntityKnucklesSpit extends EntityLlamaSpit {
    ...
    public EntityKnucklesSpit(World worldIn, EntityKnuckles p_i47273_2_)
    {
        super(worldIn);
    ...
    private void restoreOwnerFromSave()
    {
        if (this.ownerNbt != null && this.ownerNbt.hasUniqueId("OwnerUUID"))
        {
            ...
            for (EntityKnuckles knuckles : this.world.getEntitiesWithinAABB(EntityKnuckles.class, this.getEntityBoundingBox().grow(15.0D)))
            {
                if (knuckles.getUniqueID().equals(uuid))
                {
                    this.owner = knuckles;
                    break;
                }
